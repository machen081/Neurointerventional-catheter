...
# ==================== 会话状态 ====================
CURRENT_VERSION = "v15_manual_fix"
...
with st.sidebar:
    ...
    if st.button("恢复示例数据"):
        keys_to_clear = [k for k in list(st.session_state.keys())
                         ...
                         or k in ("ea_corr_input", "kp_corr_input", "span_L_input",
...
- **轴向位置滑块**：选择导管的某一截面位置，所有"当前截面"结果随滑块变化
...
- **斜线填充**：编织层
- **叉线填充**：弹簧圈
- **点线填充**：热熔填充层（该位置缺少编织层或弹簧圈，程序自动用热熔材料填充）
- **纯色**：普通材料层（PTFE、热熔层等）
...
**读图要点**：
- 找出"主力层"：例如编织层可能贡献 90% 的 EI，弹簧圈可能贡献 80% 的 Kp
- 某层贡献接近 0 说明它在该位置几乎不起作用（模量太低或截面积太小）
...
| **Bending Yield Force (3-pt)** | N | 三点弯曲换算的屈服力（跨距 L） |
...
- **Axial Tensile Force**（紫）：沿长度各位置能承受的最大拉力
- **Bending Yield Moment & Force**（青 + 橙）：双轴图，左轴 My（N·mm），右轴三点弯曲力（N）
- **Collapse Force**（黄）：沿长度各位置的压扁屈服力
...
- 对应最小的那一层就是**控制层**（谁先屈服谁控制）
- 表格里可以看到每层的 E_z、r_out、σ_uts 和算出的 My

**为什么不是最外层控制？** 弯曲时各层**曲率 κ 相同**，层表面应力 σ_i = E_z,i · κ · r_out,i。

令 σ_i = σ_uts,i，得到该层屈服时对应的整体弯矩：

    M_i = σ_uts,i · EI_total / (E_z,i · r_out,i)

**谁的 M_i 最小，谁控制弯曲屈服。** M_i 最小的条件是：
- σ_uts 小 → M_i 小（倾向控制）
- E_z 大 → M_i 小（倾向控制）
- r_out 大 → M_i 小（倾向控制）

最外层只满足 r_out 大这一条，但如果它的 σ_uts 不低、E_z 又小，就不一定控制。

## 4.4 压扁屈服候选层表

类似，列出**每层单独达到抗拉强度时所需的整体压扁力 Fc**。

- 取最小值作为整体压扁屈服力
- 最小值的层是**控制层**
- 表格列出每层的 E_theta、t、σ_uts 和 Fc

**为什么弹簧圈常常是控制层？** 各层**共同变形**，应变 ε 相同。第 i 层表面应力：
...
**谁的 ε_y 最小，谁就最先达到自己的强度极限。**

以默认数据为例：

| 层 | E_θ (MPa) | σ_uts (MPa) | 屈服应变 ε_y |
|---|---|---|---|
| Hot Melt | 12 | 15 | 1.25（125%） |
| 弹簧圈 | 25881 | 2200 | **0.085（8.5%）** |
| Coil | 400 | 106.2 | 0.266（26.6%） |

弹簧圈的屈服应变最小（8.5%），所以是压扁屈服的控制器。

**物理直觉**：三个人一起被压缩（位移相同），最"硬"的那个表面应力最大。但如果他的屈服应变 σ_uts/E 反而最小，就会最先达到自己的强度极限。弹簧圈就是这样——环向刚度极高（E_θ 大），但丝材强度没能同比例跟上，所以屈服应变最小。

## 4.5 轴向拉力贡献表

列出**每层对轴向拉力 Fu 的贡献**。

- **方法列**：显示每层用的是 "σ·A·Vf" 还是 "弹簧公式"
- 弹簧圈用**弹簧拉伸公式**，因为螺旋结构抗拉不能直接用材料强度公式
- 普通材料、编织层用 σ_uts × 面积 × 体积分数

**弹簧圈贡献通常很小（< 1 N）**：因为螺旋结构的丝与轴向接近垂直，轴向拉力几乎不直接拉伸丝本身，而是通过"把弹簧拉直"来实现，需要很大的位移才有小量的力。
...
|---|---|
| Layer | 层名称 |
| Type | 层类型（普通材料/编织层/弹簧圈/热熔填充） |
| r_in, r_out | 内半径、外半径（mm） |
| E_z | 等效轴向模量（MPa） |
| E_theta | 等效环向模量（MPa） |
| UTS | 抗拉强度（MPa） |
| V_f | 丝材体积分数（%） |
...
- **弹性模量 E_z**：材料弹性模量。PTFE 约 400~700 MPa，Pebax 2533 约 12 MPa，Pebax 7233 约 50 MPa
- **抗拉强度 σ_uts**：材料拉断前的最大应力。PTFE 约 80~106 MPa，Pebax 约 10~30 MPa

## 编织层

- **扁丝宽度 w、厚度 t**：单根扁丝的尺寸（mm）
- **股数 N**：每个方向的丝束数
- **每束根数 n_s**：每束里并排的扁丝数
- **PPI**：每英寸交叉数（编织密度）
- **丝材模量 E_f**：单根丝材的弹性模量（不锈钢约 193 GPa）
- **丝材抗拉强度**：单根丝材的抗拉强度（不锈钢约 2000~2500 MPa）
- **原始基体体积分数**：编织层在渗入热熔前已有的基体体积占比（通常填 0）

**编织角自动计算**：程序根据 PPI、股数、内外半径自动算编织角 α。

## 弹簧圈

- **丝径 d**：单根丝的直径（mm）
- **螺距 p**：相邻两圈丝之间的距离（mm）。**螺距越大，弹簧圈越稀疏，等效模量越低**
- **丝材模量、抗拉强度**：丝材本身参数
- **原始基体体积分数**：同编织层
...
|---|---|
| 层间滑移 | 折减 0.4~0.7 |
| 几何非线性（大变形） | 折减 0.3~0.6 |
| 材料屈服 | 载荷越大折减越明显 |
| PTFE 内衬尺寸效应 | 有效模量低于块体值 |
...
1. **先保持 1.0**，看理论值
2. 做一次实验（拉伸 + 平板压缩），得到实测 EA 和 Kp
3. 修正系数 = 实测值 / 理论值
4. 把系数填进侧边栏（例如 0.4）
5. 从此工具就"对准"了，可以用来预测同类结构

## 注意

- EA 和 Kp 的修正系数**分开填**（因为两者机理不同）
- EI 没有修正系数（因为实验标定 EI 比较困难）
- 修正系数适用于**结构相近**的导管，结构变化大时需要重新标定
...
**刚度决定好不好用，强度决定安不安全。** 两者完全独立。改了模量（E）只影响刚度；改了强度（σ_uts）只影响强度。**不要把两者混用。**
...
**Q1：算出来 Kp 太大，感觉不现实？**
A：Kp 是**线性小变形刚度**，变形超过 20% 后非线性失效。真实值需要乘修正系数 0.3~0.5。

**Q2：为什么 Coil 层改了模量，强度 Fu 没变？**
A：Fu 只取决于**抗拉强度 σ_uts**，与弹性模量 E 无关。

**Q3：为什么弹簧圈对 Fu 贡献很小？**
A：弹簧圈是螺旋结构，轴向拉伸不能直接用材料强度公式。程序用弹簧拉伸公式单独算，结果通常 < 1 N。

**Q4：为什么弯曲屈服不是最外层控制？**
A：各层曲率相同，层表面应力 σ_i = E_z · κ · r_out。令 σ_i = σ_uts,i 得到该层屈服时的整体弯矩 M_i = σ_uts,i · EI_total / (E_z,i · r_out,i)。**谁 M_i 最小谁控制。** 最外层只满足 r_out 大，但如果它的 σ_uts 不低、E_z 又小，就不一定控制。

**Q5：为什么弹簧圈是压扁屈服的控制层？**
A：各层共同变形，应变 ε 相同。第 i 层屈服应变为 ε_y,i = σ_uts,i / E_θ,i。弹簧圈的 E_θ 极高（25881 MPa）而 σ_uts 只有 2200 MPa，所以 ε_y ≈ 8.5%，是所有层中最小的——**屈服应变最小 → 最先达到自己的强度极限**。不是因为它"最硬所以坏得早"，而是因为它的强度/刚度比最低。

**Q6：改了参数图表没更新？**
A：改完表格后**按一次 Enter**，或者点侧边栏底部「🔄 强制刷新计算」。
...
if layers:
    ...
    st.caption("弹簧圈用弹簧拉伸公式计算，其余用 σ·A·Vf。弹簧圈贡献通常很小（< 1 N）。")
    fu_rows = []
    for i, l in enumerate(layers):
